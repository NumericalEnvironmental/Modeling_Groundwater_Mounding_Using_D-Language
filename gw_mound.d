...
class Model {
    ...
    int x_int, y_int, track_dir;
    bool gridder, vectors, particle_trace, monitor;
    string line_input;
    string[] parsed_input;  
    this(){     // constructor
        ...
        while (!input_file.eof()) {
            ...
            switch (parsed_input[0]) {
                ...
                case "gridder":                             // whether or not to model groundwater elevation across grid (boolean)
                    this.gridder = to!bool(parsed_input[1]);
                    break;                      
                case "flow_vectors":                            // whether or not to model groundwater flow vectors across grid (boolean)
                    this.vectors = to!bool(parsed_input[1]);
                    break;              
                ...
                case "x_int":                               // grid discretization density, x-axis
                    this.x_int = to!int(parsed_input[1]);
                    break;  
                case "y0":                              // grid origin, y-axis
                    this.y0 = to!double(parsed_input[1]);
                    break;      
                case "yf":                              // grid end, y-axis
                    this.yf = to!double(parsed_input[1]);
                    break;  
                case "y_int":                               // grid discretization density, y-axis
                    this.y_int = to!int(parsed_input[1]);
                    break;  
                case "particle_trace":                          // whether or not to employ particle tracking (boolean)
                    this.particle_trace = to!bool(parsed_input[1]);
                    break;      
                case "track_direction":                         // direction for particle tracking (1=forward; -1=backward)
                    this.track_dir = to!int(parsed_input[1]);
                    break;                      
                case "dt":                              // time-step size for particle tracking and time series
                    this.dt = to!double(parsed_input[1]);
                    break;  
                case "monitor":                             // whether or not to model time series (boolean)
                    this.monitor = to!bool(parsed_input[1]);
                    break;                      
                default:                                // maximum particle displacement, per time step
                    this.d_max = to!double(parsed_input[1]);                
                    break;
                }
            }
        input_file.close(); 
...
class Tracker {
    ...
    double[] y;
    int track_dir;          // direction of particle movement with respect to groundwater velocity (e.g., forward or backward)
    
    this(int track_dir){                // constructor
    ...
    private Tuple!(double, double) Velocity(double x, double y, double t, Aquifer aquifer, Mound[] basin, Well[] well){
        ...
        return tuple(vx*track_dir, vy*track_dir);   
        }
    ...
    public void Track(Model model, Aquifer aquifer, Mound[] basin, Well[] well){
        ...
        for (int i = 0; i < x.length; ++i){
            ...
            while (live == true){
                ...
                // deactivate particle if applicable time boundary (i.e., forward or backward) reached
                if ((t >= model.t_end) && (track_dir == 1)){live = false;}                
                if ((t <= 0.) && (track_dir == -1)){live = false;}
                ...
                // deactivate particle if it is within d_max from the center of a recharge basin
                foreach(pond; basin){
                    r = Dist(x[i], y[i], pond.xm, pond.ym);
                    if (r <= model.d_max) {
                        live = false;                   // finished tracking this particle, if it's close to the center of recharge basin (for backward tracking)
                        break;
                        }           
                    }           
...
class Grid {
    ...
    // x-y grid for calculating head distribution (and flow vectors, if selected)
    ...
    public void FlowVectors(Model model, Aquifer aquifer, Mound[] basin, Well[] well){
        // compute groundwater flow vectors on grid of points
        double delta = 0.001;       
        for (int i = 0; i < x.length; ++i){
            vx[i] = -aquifer.K * (h_tot(x[i]+0.5*delta, y[i], model.t_end, aquifer, basin, well)
                - h_tot(x[i]-0.5*delta, y[i], model.t_end, aquifer, basin, well)) / (delta * aquifer.phi);
            vy[i] = -aquifer.K * (h_tot(x[i], y[i]+0.5*delta, model.t_end, aquifer, basin, well)
                - h_tot(x[i], y[i]-0.5*delta, model.t_end, aquifer, basin, well)) / (delta * aquifer.phi);  
            }
        }           
        
    public void WriteHeadOutput(){
    ...
    public void WriteVectorsOutput(){
        // groundwater flow vector output at the end of the simulation
        string line_string;
        auto output_file = File("vectors_out.txt", "w");
        line_string = "x" ~ "\t" ~ "y" ~ "\t" ~ "vx" ~ "\t" ~ "vy";             // write header
        output_file.writeln(line_string);
        for (int i = 0; i < x.length; ++i){
            line_string = to!string(x[i]) ~ "\t" ~ to!string(y[i]) ~ "\t" ~ to!string(vx[i]) ~ "\t" ~ to!string(vy[i]);
            output_file.writeln(line_string);
            }           
        output_file.close();        
        }       
    ...
    public double dh_time_int(double r, double t, double b){
        ...
        while (i < well_history.time.length){
            if (t > well_history.time[i]) {
                ...
                if (i > 0){dh -= s(r, dt, b, well_history.flux[i-1]);}
                }
...
void main(){
    ...
    model = new Model();                                    // model parameters
    writeln("Read model parameters.");
    
    aquifer = new Aquifer();                                // aquifer properties
    ...
    if (model.gridder==true){                               // set up grid class; calculate heads and write output
        ...
        grid.WriteHeadOutput();
        if (model.vectors==true){
            writeln("Processing groundwater flow vectors.");    // add flow vectors, if indicated
            grid.FlowVectors(model, aquifer, basin, well);
            grid.WriteVectorsOutput();
            }
        }
    
    if (model.monitor == true){                             // perform time series calculations at monitoring points
        writeln("Processing time series at monitor points.");
        Monitor monitor = new Monitor();
        monitor.WaterLevel(model, aquifer, basin, well);
        }
    
    if (model.particle_trace == true){                      // perform particle tracking, if indicated
        writeln("Tracking particles ...");
        Tracker trace = new Tracker(model.track_dir);
